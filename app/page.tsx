...
export default function Home() {
    ...
    const autocomplete = new window.google.maps.places.Autocomplete(
      addressInputRef.current,
      {
        ...
        bounds: new window.google.maps.LatLngBounds(
          { lat: 49.05, lng: -122.75 },
          { lat: 49.25, lng: -122.50 }
        ),
        strictBounds: false,
      }
    )
  ...
  async function handleEmailReport() {
    ...
    try {
      await fetch('/api/email-report', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ address: lead.address, name: lead.name, phone: lead.phone, email: lead.email, bcaData, narrative, checkedRenos }),
      })
      setEmailSent(true)
    } catch {}
  ...
  // Build the property subtitle line for Gate 3 header
  const propertySubtitle = (() => {
    ...
    if (bcaData.floorArea && bcaData.floorArea !== '-' && bcaData.floorArea !== 'nan') {
      parts.push(`${Number(bcaData.floorArea).toLocaleString()} sq ft`)
    }
        ...
        <div>
          <div style={{ background: '#0A1628', padding: '60px 24px 80px', textAlign: 'center' }}>
            ...
            <p style={{ color: '#94A3B8', fontSize: 17, maxWidth: 540, margin: '0 auto 40px' }}>
              You bought in Willoughby when others weren't sure. Now it's time to find out what that decision is actually worth.
            </p>
            <div style={{ maxWidth: 560, margin: '0 auto', display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <input
                ...
                style={{ flex: 1, minWidth: 260, padding: '16px 20px', fontSize: 16, border: '2px solid #1E3A5F', borderRadius: 10, background: '#0D1F3C', color: '#fff', outline: 'none' }}
              />
              <button
                onClick={handleAddressSubmit}
                disabled={loading || !address.trim()}
                style={{ padding: '16px 28px', background: loading ? '#666' : '#0D9488', color: '#fff', fontWeight: 700, fontSize: 16, border: 'none', borderRadius: 10, cursor: loading ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap' }}
              >
                {loading ? 'Checking...' : 'See my estimate →'}
              </button>
            </div>
            <p style={{ color: '#4A6080', fontSize: 13, marginTop: 12 }}>No sign-up required to see your market range</p>
          </div>

          <div style={{ background: '#fff', borderBottom: '1px solid #E2E8F0' }}>
            <div style={{ maxWidth: 800, margin: '0 auto', display: 'flex', flexWrap: 'wrap', justifyContent: 'center' }}>
              {[
          ...
          <div style={{ maxWidth: 860, margin: '60px auto', padding: '0 24px' }}>
            <h2 style={{ textAlign: 'center', fontSize: 26, fontWeight: 700, marginBottom: 40 }}>What you get — for free, in seconds</h2>
          ...
          <footer style={{ background: '#0A1628', color: '#64748B', textAlign: 'center', padding: '32px 24px', fontSize: 13 }}>
            ...
            <p style={{ marginTop: 12 }}><a href="/privacy" style={{ color: '#4A6080', textDecoration: 'underline' }}>Privacy Policy</a></p>
          </footer>
        </div>
      )}
        ...
        <div style={{ maxWidth: 640, margin: '48px auto', padding: '0 16px' }}>
          <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #E2E8F0', overflow: 'hidden' }}>
            <div style={{ background: '#0A1628', padding: '24px 28px' }}>
              <p style={{ color: '#C8952A', fontSize: 12, fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 6 }}>Property detected</p>
            ...
            <div style={{ padding: '28px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 28 }}>
                <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 10, padding: 16, textAlign: 'center' }}>
                  ...
                  <div style={{ fontWeight: 800, fontSize: 20, color: '#0F2B5B' }}>{bcaData ? fmt(bcaData.assessedTotal) : '$1,701,000'}</div>
                ...
                <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 10, padding: 16, textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ fontWeight: 800, fontSize: 18, color: '#0F2B5B', filter: 'blur(7px)', userSelect: 'none' }}>{bcaData ? fmt(bcaData.estimateLow) : '$1,650,000'}</div>
                  <div style={{ color: '#94A3B8', fontSize: 12, marginTop: 4, filter: 'blur(5px)', userSelect: 'none' }}>Based on $/sqft</div>
                  <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ fontSize: 20 }}>🔒</span></div>
                </div>
                <div style={{ background: '#FEF9EC', border: '1px solid #C8952A', borderRadius: 10, padding: 16, textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ fontWeight: 800, fontSize: 18, color: '#C8952A', filter: 'blur(7px)', userSelect: 'none' }}>{bcaData ? fmt(bcaData.equityGain) : '$1,346,058'}</div>
                  <div style={{ color: '#92600A', fontSize: 12, marginTop: 4, filter: 'blur(5px)', userSelect: 'none' }}>Tax-free</div>
                  <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ fontSize: 20 }}>🔒</span></div>
                </div>
              </div>
              <p style={{ textAlign: 'center', color: '#64748B', fontSize: 14, marginBottom: 8 }}>Most original Willoughby owners are surprised — not by the number, but by what it means for the next chapter.</p>
              <p style={{ textAlign: 'center', color: '#94A3B8', fontSize: 13, marginBottom: 24 }}>Enter your name and phone to unlock your full estimate</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div>
                  <input
                    value={lead.phone}
                    onChange={e => { setLead(p => ({ ...p, phone: formatPhone(e.target.value) })); if (phoneError) setPhoneError('') }}
                    placeholder="Phone number (e.g. 604-555-0123)"
                    type="tel"
                    style={{ width: '100%', padding: '14px 16px', border: `1.5px solid ${phoneError ? '#EF4444' : '#CBD5E1'}`, borderRadius: 8, fontSize: 16, outline: 'none', boxSizing: 'border-box' }}
                  />
                ...
                <button
                  ...
                  style={{ padding: '16px', background: loading ? '#94A3B8' : '#0D9488', color: '#fff', fontWeight: 700, fontSize: 16, border: 'none', borderRadius: 10, cursor: loading ? 'not-allowed' : 'pointer' }}
                >
                  {loading ? 'Preparing your report...' : 'Unlock my estimate →'}
                </button>
                <p style={{ textAlign: 'center', color: '#94A3B8', fontSize: 12 }}>No spam. No pressure. Your details are used only to prepare your personalized report.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 16px', display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 320px', gap: 24, alignItems: 'start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #E2E8F0', overflow: 'hidden' }}>
              <div style={{ background: '#0A1628', padding: '20px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                ...
                <span style={{ color: '#64748B', fontSize: 13 }}>{propertySubtitle}</span>
              </div>
            ...
            <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #E2E8F0', padding: '24px 28px' }}>
              ...
              <p style={{ color: '#64748B', fontSize: 14, marginBottom: 20 }}>Have you renovated? Select what applies and we'll adjust your estimated range.</p>
              ...
              {checkedRenos.length > 0 && (
                <div style={{ marginTop: 20, padding: '16px 20px', background: '#F0FDFA', borderRadius: 10, border: '1px solid #0D9488' }}>
                  <p style={{ color: '#0A1628', fontWeight: 700, fontSize: 15 }}>Renovation-adjusted estimate: {fmt(adjLow)} – {fmt(adjHigh)}</p>
                  <p style={{ color: '#64748B', fontSize: 13, marginTop: 4 }}>Buyer premium for your upgrades: +{fmt(renoAddLow)}–{fmt(renoAddHigh)} above base estimate</p>
                </div>
              )}
            </div>
            ...
            <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #E2E8F0', padding: '24px 28px' }}>
              ...
              <p style={{ color: '#64748B', fontSize: 14, marginBottom: 16, lineHeight: 1.7 }}>
                Most original owners assume moving is expensive and disruptive. Here's the realistic math on a {bcaData ? fmt(Math.round((bcaData.estimateLow + bcaData.estimateHigh)/2)) : '$1,700,000'} sale:
              </p>
              ...
              <div style={{ marginTop: 16, padding: '14px 16px', background: '#F0FDF4', borderRadius: 8, border: '1px solid #166534' }}>
                <p style={{ color: '#166534', fontSize: 13, fontWeight: 600 }}>Total transition costs represent less than 3% of your estimated net proceeds — far less than most owners expect.</p>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, position: 'sticky', top: 20 }}>
            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E2E8F0', padding: '20px' }}>
              ...
              <p style={{ color: '#334155', fontSize: 14, lineHeight: 1.6 }}>There is currently strong buyer demand in Willoughby for detached homes in the $1.3M–$1.8M range. I work with buyers actively looking in this area.</p>
            </div>
            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E2E8F0', padding: '20px' }}>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
                {['Families relocating from Metro Vancouver — pre-approved $1.3M–$1.8M','Looking for move-in ready — not planning renovations','Prioritize school catchment, suite income, and garage','Typically need 30–45 day completion timeline'].map((pt, i) => (
            ...
            <div style={{ background: '#0A1628', borderRadius: 12, padding: '20px', border: '1px solid #1E3A5F' }}>
              ...
              <p style={{ color: '#94A3B8', fontSize: 13, lineHeight: 1.6 }}>There are very few comparable detached homes available in Willoughby at this moment. Homeowners who list first in a low-supply market consistently capture the strongest offers.</p>
            </div>
            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E2E8F0', padding: '20px' }}>
              <h4 style={{ fontWeight: 700, fontSize: 15, marginBottom: 6, color: '#0A1628' }}>Get the full report as a PDF</h4>
              <p style={{ color: '#64748B', fontSize: 13, marginBottom: 16, lineHeight: 1.5 }}>Includes your renovation-adjusted estimate, net-in-pocket number, full comp breakdown, and move cost calculator.</p>
              {emailSent ? (
                ...
                <>
                  <input value={lead.email} onChange={e => setLead(p => ({ ...p, email: e.target.value }))} placeholder="your@email.com" type="email" style={{ width: '100%', padding: '12px 14px', border: '1.5px solid #CBD5E1', borderRadius: 8, fontSize: 15, marginBottom: 10, boxSizing: 'border-box', outline: 'none' }} />
                  ...
                  <p style={{ color: '#94A3B8', fontSize: 12, marginTop: 10, textAlign: 'center' }}>Kamran Khan will follow up personally within 24 hours</p>
                </>
              )}
            </div>
        ...
        <div style={{ maxWidth: 560, margin: '80px auto', padding: '0 16px', textAlign: 'center' }}>
          <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #E2E8F0', padding: '48px 40px' }}>
            ...
            <div style={{ background: '#FEF9EC', border: '1px solid #C8952A', borderRadius: 10, padding: '20px', marginBottom: 24 }}>
              ...
              <a href="tel:+12366602594" style={{ display: 'block', padding: '12px', background: '#0A1628', color: '#fff', fontWeight: 700, borderRadius: 10, textDecoration: 'none', fontSize: 15 }}>Call +1-236-660-2594</a>
            </div>
